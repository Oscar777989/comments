...
if(preg_match('/^[A-Za-záéíóúÁÉÍÓÚñÑ\s:;,()$%@#¿?¡!&quot;]+$/',$_POST["content"])){

           $fields = array(
            "id_user_publication" => $_SESSION['user']->id_user,
            "content_publication" => $_POST["content"],
            "verif_publication" => 1,
            "date_updated_publication" => $now,
            "date_created_publication" => $now
           );



          $url = "publications?token=no&table=publications&suffix=publication&except=content_publication";
          $method = "POST";
          $publication = CurlController::request($url,$method,$fields);

          if ($publication->status == 200) {
            print_r($publication->status);
          }else {
            print_r($publication->status);
          }
